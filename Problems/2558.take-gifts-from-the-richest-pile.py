...
class Solution(object):
    def pickGifts(self, gifts, k):
        ...
        negative_gifts = [-g for g in gifts]
        heapq.heapify(negative_gifts)
        
        for _ in range(k):
            n = -heapq.heappop(negative_gifts)
            heapq.heappush(negative_gifts, -math.floor(math.sqrt(n)))
            
        return int(-sum(negative_gifts))
